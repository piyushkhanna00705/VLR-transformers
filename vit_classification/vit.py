...
class ViT(nn.Module):
    ...
    def patchify(self, images):
        ...
        patched_image = unfold(images).view(N, C, self.patch_dim, self.patch_dim, -1)
        images = patched_image.permute(0, 4, 1, 2, 3).flatten(2)
        return images
    ...
    def forward(self, images):
        ...
        patch = self.patchify(images)
        patches_emb = self.patch_embedding(patch)
        
        
        cls_tokens = self.cls_token.repeat(patches_emb.shape[0], 1, 1)
        output = torch.cat((cls_tokens, patches_emb), dim=1) 
